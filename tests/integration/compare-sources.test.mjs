...
describe("Route 701: OTP vs FIWARE comparison", () => {
  ...
  it("vehicle counts are similar", () => {
    ...
    assert.ok(
      diff <= allowedDiff,
      `Counts should be within 30% of each other (diff=${diff}, max=${maxCount})`
    );
  });
  ...
  it("trip IDs match between sources", () => {
    ...
    console.log(
      `  OTP-only trips:   ${otpOnly.length} ${otpOnly.length > 0 ? otpOnly.slice(0, 3).join(", ") : ""}`
    );
    console.log(
      `  FIWARE-only trips: ${fiwareOnly.length} ${fiwareOnly.length > 0 ? fiwareOnly.slice(0, 3).join(", ") : ""}`
    );
  });
  ...
  it("vehicle IDs match between sources", () => {
    ...
    console.log(
      `  FIWARE-only vehicles: ${fiwareOnly.length} ${fiwareOnly.slice(0, 5).join(", ")}`
    );
  });
  ...
  it("GPS positions are close for matched vehicles", () => {
    ...
    for (const fb of fiwareBuses) {
      ...
      diffs.push({
        vehicle: fb.vehicleId,
        distMeters: Math.round(distMeters),
        otpLat: ob.lat,
        fiwareLat: fb.lat,
        otpLon: ob.lon,
        fiwareLon: fb.lon,
        headingDiff: Math.abs((ob.heading || 0) - fb.heading),
        speedDiff: Math.abs((ob.speed || 0) - fb.speed),
      });
    }
    ...
    if (diffs.length > 0) {
      ...
      diffs
        ...
        .forEach((d) =>
          console.log(
            `    Vehicle ${d.vehicle}: ${d.distMeters}m apart, heading ±${d.headingDiff}°, speed ±${d.speedDiff.toFixed(1)}`
          )
        );
    }
  });
