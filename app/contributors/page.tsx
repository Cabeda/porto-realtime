    ...
    <div className="min-h-screen bg-(--color-surface-sunken) text-(--color-content)">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="rounded-xl border border-(--color-border) bg-(--color-surface) p-4 mb-6">
          <h2 className="text-sm font-semibold mb-3 text-(--color-content-secondary)">Badges</h2>
            ...
            {Object.values(BADGES).map((b) => (
              <div key={b.id} className="flex items-start gap-2 text-sm">
                ...
                <div>
                  ...
                  <p className="text-xs text-(--color-content-secondary)">{b.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Leaderboard */}
        {isLoading && (
          <div className="text-sm text-(--color-content-secondary) text-center py-12">Loading…</div>
        )}

        {!isLoading && data?.contributors?.length === 0 && (
          <div className="text-sm text-(--color-content-secondary) text-center py-12">
        ...
        {data?.contributors && data.contributors.length > 0 && (
          <div className="rounded-xl border border-(--color-border) overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-(--color-surface) border-b border-(--color-border) text-(--color-content-secondary) text-xs">
              ...
              <tbody>
                {data.contributors.map((c) => (
                  <tr
                    key={c.rank}
                    className="border-b border-(--color-border) last:border-0 bg-(--color-surface-raised) hover:bg-(--color-surface) transition-colors"
                  >
                    <td className="px-4 py-3 font-mono text-(--color-content-secondary)">
                    ...
                    <td className="px-4 py-3">
                      {c.badges.length > 0 ? (
                        ...
                        <span className="text-(--color-content-secondary) text-xs">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-right font-medium">{c.reviewCount}</td>
                    <td className="px-4 py-3 text-right text-(--color-content-secondary)">
                      {c.totalVotes}
                    </td>
                  </tr>
                ))}
        ...
        <p className="mt-4 text-xs text-(--color-content-secondary)">
