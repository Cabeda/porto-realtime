...
function BikeParkReviewsContent() {
  ...
  const { data: detail } = useSWR<TargetDetail>(
    parkId ? `/api/feedback/rankings?type=BIKE_PARK&targetId=${encodeURIComponent(parkId)}` : null,
    jsonFetcher,
  ...
  const { data: feedbackList, mutate } = useFeedbackList("BIKE_PARK", parkId || null, page, 20, sort);
  ...
  if (!parkId) {
    return (
      <div className="min-h-screen bg-surface-sunken flex items-center justify-center p-4">
        <p className="text-content-muted">Parque de bicicletas não encontrado</p>
      </div>
    );
  }
...
export default function BikeParkReviewsPage() {
  return (
    <Suspense
      fallback={
